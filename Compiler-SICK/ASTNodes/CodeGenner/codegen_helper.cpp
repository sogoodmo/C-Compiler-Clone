...
llvm::Value *GetBool(llvm::Value *val, llvm::Type *type, std::string loopStr, TOKEN tok)
{
	if (type->isFloatTy())
	{
		return Builder.CreateFCmpONE(val, GetConstant(VAR_TYPE::FLOAT_TYPE, 0.0f, false), loopStr);
	}
	else if (type->isIntegerTy(32))
	{
...
llvm::Value *ImplicitCasting(llvm::Value *val, llvm::Type *newType, TOKEN tokInfo, std::string optionalError)
{
	...
	if (oldType->isFloatTy() && newType->isIntegerTy(1))
	{
		...
		return Builder.CreateFCmpONE(val, GetConstant(VAR_TYPE::FLOAT_TYPE, 0.0f, false), tokInfo.lexeme.c_str());
	}
